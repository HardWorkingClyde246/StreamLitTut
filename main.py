...
candidate_id = st.sidebar.number_input("Candidate ID", min_value=1, step=1, value=1)
...
if uploaded_file:
    ...
    try:
        ...
        # Save results to CSV
        candidates_info = [
            {'CandidateID': candidate_id, 'CandidateName': candidate_name, 'CandidateEmail': candidate_email,
             'Position': position, 'Summarisation': summary, 'Keywords': ", ".join(keyword_list)}
        ]
        df = pd.DataFrame(candidates_info)
        csv_file_name = 'candidates_results.csv'
